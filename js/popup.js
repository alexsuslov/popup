...
(function() {
  ...
  Popup = (function() {
    ...
    function Popup(options, elem) {
      ...
      $(this.elem).after("<div id='fog'></div>");
    ...
    Popup.prototype.display = function() {
      ...
      $(this.elem).find('#workspace').html(['<iframe width=100% height="' + this.size[2] + '" frameborder=0 src="' + this.url + '">', '</iframe>'].join(''));
      if (!this.fog) {
        this.fog = $('#fog');
      }
      this.fog.addClass('fog');
      return this.fog.css('height', window.innerHeight);
    };

    Popup.prototype.clean = function() {
      $(this.elem).find('#workspace').html('');
      this.fog.removeClass('fog');
      return this.fog.css('height', 0);
    };
